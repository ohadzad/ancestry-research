...
- [המועמד — מגדים 1954](#s3) — הרישום, מה תומך, מה עומד נגד, והמועמד השני שנשלל לפי מספר זהות.
- [מועמדים שנשללו](#s4) — שמונה רשומות והנימוק לכל אחת.
...
- מועמד שני באותו כינוי ובאותו שם משפחה נבדק ונשלל לפי מספר זהות — [3.4](#s3-4); הצירוף כינוי + שם משפחה חוזר אפוא אצל אדם אחר, ולכן אינו מזהה.
...
**מה תמך:** אותו כינוי, אותו שם משפחה ("בן הראש" — כתיב שנוסף למוסכמות [9.3](#s9-3) במהדורה 11); ירושלים — עיר משפחתה של מרים מ-1956. **ההכרעה (12.09.2026):** המשפחה מסרה את מספר הזהות של מרדכי מתעודותיו — והוא **אינו** 6419140 (המספר עצמו נשמר ב-THREADS, M-T1, ואינו מתפרסם). מספר הזהות ברשומה הוא נתון מאומת, ולכן המועמד **נשלל**: "בן הראש מרדושה" מירושלים הוא אדם אחר, נושא אותו כינוי ואותו שם משפחה.

**סיכום פרק 3:** רישום מגדים 1954 חוזר להיות המועמד היחיד — **ככל הנראה**, לא יותר: הכינוי אושר (בשאלה מובילה) ונפוץ, כפי שמוכיח המועמד שנשלל; המשפחה אינה מכירה מגדים ולא שינוי שם; ואין מסמך שקושר את מרדכי למגדים. מה שיכריע: תמצית רישום של מרדכי ממרשם האוכלוסין "בתוספת פרטי כניסה לארץ, ישוב לידה ושמות קודמים" — קרוב מדרגה ראשונה זכאי לה בלשכה ([פרק 7](#s7), פעולה 1) — או תיקו האישי בארכיון צה"ל (כתובת 1954).
...
| ילקוט הפרסומים 1144 (30.12.1964), עמ' 901, ירושלים, רשימה 9/63 — "בן הראש מרדושה → בן הראש מרדכי"[[13]] | מס' זהות 6419140 · שינוי שם אושר 9/1963 | מספר הזהות של מרדכי (משפחה, מתעודותיו) שונה — [סעיף 3.4](#s3-4) | **נשלל** |
...
- **יקוט (פנינה) בן הרוש** — אמו; ילידת 1918 (רשימת 1956); "יקוט" במרוקו ובמסמכים, "פנינה" בישראל (משפחה בלבד — ככל הנראה); עלתה עם יונה; תל אביב–יפו; קבורה בהר המנוחות (משפחה). פטירה — **טעון אימות**.
  ...
  - **מרדכי בן הרוש** — 1931 (משפחה; כנראה כך גם במשרד הפנים) – 2010 (משפחה). כינויו "מרדושי". עלה 1947/48 לבדו בעליית הנוער, בגיל 16–17, והתגייס מיד (משפחה; מתיישב עם הרשומות). ככל הנראה "מרדושי בנערוש" ממגדים (1954); "מרדושה בן הראש" מירושלים (1963) נשלל לפי מספר זהות.
...
| # | פעולה | מה זה ישנה | מי |
|---|---|---|---|
| 1 | **תמצית רישום של מרדכי ממרשם האוכלוסין** "בתוספת פרטי כניסה לארץ, ישוב לידה ושמות קודמים" (נוהל רשות האוכלוסין 2.17.0001, סעיף 4.7) — מכריעה את מועמד מגדים | "שמות קודמים: בנערוש" + תאריך שינוי 12.12.1954 = זיהוי; היעדרו = שלילה. מגישים בלשכה: בת זוג או ילד (קרבה ראשונה), טופס מר/2, חינם. במקביל: תעודת פטירה (אונליין) — שמות ההורים ותאריך הלידה הרשום | משפחה (קרוב מדרגה ראשונה) |
...
הרשימה המלאה ב[אינדקס המקורות](#index). ערך [1](#src-1) הוא המועמד לרשומת שינוי השם, וערך [13](#src-13) — מועמד שני באותו כינוי, שנשלל לפי מספר זהות; ערכים [2](#src-2), [3](#src-3), [6](#src-6) ו-[8](#src-8) — המועמדים שנשללו; ערכים [5](#src-5) ו-[9](#src-9) — הסריקות השליליות; ערך [7](#src-7) — מקור העזר על מגדים.
