...
GALLERY = (
    ...
    (F.thumb(ROOT, 'docs/ev_wedding_1960_hebrew.jpg', 440), 'docs/1960-09-12_wedding_invitation.jpg', 'הזמנה לחתונת מרים ומרדכי — תל אביב, 12.9.1960; הורי הכלה יקוט ושלום דהן, הורי החתן יקוט ויונה בן-הרוש'),
...
STORY = Story(
    lede=(
        ...
        'היא עולה לישראל עם הוריה ואחיותיה באונייה <b>"ירושלים"</b> — ובאותו עמוד של רשימת '
        'העולים גם סבה, דודיה ובנות דודתה. הדרך עברה במרסיי, בקריית שמונה ובירושלים; '
        'ב-1960 נישאה בתל אביב ל<b>מרדכי בן הרוש</b>. הדף הזה מספר מה שהמסמכים מראים, '
        'לפי סדר הזמן.'),
    verdicts=(
        ...
        V('כמעט ודאי', 'כמעט ודאי', (
            ...
            ('שנת הלידה <b>1939</b>, כרשום בתעודת הזהות ובפנקס', '#s4'),
    ...
    timeline=(
        B('לפני 1939', 'משפחת <b>דהן</b> במכנאס: הסב <b>חביב</b> ובניו — <b>שלום</b>, '
                       'אביה של מרים, חייט; ולצדו אחיו <b>מכלוף</b> ו<b>שלומון</b>. '
                       'כולם יעלו ארצה יחד, באותה אונייה, ב-1956.',
          place='מכנאס', rank='מאומת', href='#s5-1',
          src=('רשימת העולים של "ירושלים", ארכיון המדינה', '#src-6')),
        B('1939', 'נולדה ב<b>מכנאס</b>, בתם של שלום דהן, חייט, ויקוט לבית אבוטבול.',
          place='מכנאס', rank='כמעט ודאי', href='#s4',
          src=('פנקס אליאנס מכנאס, 1949', '#src-1')),
        B('1949', 'נרשמת לבית הספר לבנות של <b>אליאנס</b> במכנאס — "Dahan Marie, 10 ans", '
                  'על שם ההורים Chalom ו-Yacot; מקצוע האב ומקום המגורים: חייט, המלאח הישן.',
          place='מכנאס', rank='מאומת', href='#s3-1',
          src=('פנקס אליאנס מכנאס, 1949, מס\' 1617', '#src-1')),
        B('1950 · 1953', 'אחיותיה <b>רבקה ורחל</b> נרשמות לאותו בית ספר, על שם אותם הורים.',
          place='מכנאס', rank='מאומת', href='#s3',
          src=('פנקסי אליאנס 1950 ו-1953', '#src-2')),
        B('15.1.1956', 'עולה לישראל באונייה <b>"ירושלים"</b> עם הוריה ושתי אחיותיה. '
                       'באותו עמוד של רשימת העולים: סבה <b>חביב</b>, דודיה <b>מכלוף '
                       'ושלומון</b>, ודודתו של אביה <b>צפורה</b>.',
          place='נמל חיפה', rank='מאומת', href='#s5-1',
          src=('רשימת העולים של "ירושלים", ארכיון המדינה', '#src-6')),
        B('1956', 'מהנמל נשלחה המשפחה ל<b>קריית שמונה</b>. לפי המשפחה: כחודש ב<b>מרסיי</b> '
                  'לפני ההפלגה, וכחצי שנה בקריית שמונה לפני המעבר ל<b>ירושלים</b>.',
          rank='טעון אימות', href='#s2', src=('מסירות משפחתיות, ספטמבר 2026', '#src-4')),
        B('12.9.1960', 'נישאת ב<b>תל אביב</b> ל<b>מרדכי בן הרוש</b>, יליד מכנאס 1928, '
                       'שעלה לבדו שבע שנים לפניה. החופה במועדון "ויצו" שבשכונת '
                       'מונטיפיורי; ההזמנה נושאת את שמות ארבעת ההורים.',
          place='תל אביב', rank='מאומת', href='#s5-2',
          src=('הזמנת החתונה, 12.9.1960', '#src-13')),
        B('3.3.1976', 'אמה <b>יקוט</b> נפטרת ונקברת בהר המנוחות בירושלים; אביה <b>שלום</b> '
          ...
          src=('מסירות משפחתיות, ספטמבר 2026', '#src-4')),
    ),
    ...
    docs=(
        D('docs/ev_marie_1949.jpg', '1949', 'פנקס אליאנס מכנאס — "Dahan Marie"',
          ...
          '1939 ומקצוע האב.', 'מאומת', '#s3-1', src=('ערך 1', '#src-1')),
        D('docs/ev_rebeca_1950.jpg', '1950', 'פנקס אליאנס — רבקה דהן',
          'אחותה, שנה אחריה, על שם אותם הורים.', 'מאומת', '#s3-2', src=('ערך 2', '#src-2')),
        D('docs/ev_rachel_1953.jpg', '1953', 'פנקס אליאנס — רחל דהן',
          'האחות הצעירה, 1953 — הרשומה השלישית מתוך שלוש שמצטרפות למשפחה אחת.',
          'מאומת', '#s3-3', src=('ערך 3', '#src-3')),
        D('docs/ev_shalom_1956.jpg', '15.1.1956', 'רשימת העולים של "ירושלים"',
          'רשומת העלייה: שלום (1920), יקוט (1921), מרי (1939), רבקה (1944) ורחל (1947) — '
          'ויעד הקליטה, קריית שמונה.', 'מאומת', '#s5-1', src=('ערך 6', '#src-6')),
        D('docs/ev_dahan_1956.jpg', '15.1.1956', 'שלושת בלוקי האחים באותו עמוד',
          'מכלוף על משפחתו, שלומון, והסב חביב — כל ענף דהן עלה באותה אונייה, באותו יום.',
          'מאומת', '#s5-1', src=('ערך 6', '#src-6')),
    ...
    sources_note=('כל קביעה בדף הזה נשענת על אחד המסמכים שלהלן. כל ערך מוביל אל '
                  ...
                  'ואל עותק שמור שלה.'),
    sources=(
        ('פנקס בית הספר לבנות של אליאנס במכנאס, 1949 — מרי דהן, מס\' 1617', '#src-1'),
        ('פנקס אליאנס מכנאס, 1950 — רבקה דהן, מס\' 1878', '#src-2'),
        ('פנקס אליאנס מכנאס, 1953 — רחל דהן, מס\' 3054', '#src-3'),
        ('ארכיון המדינה — רשימת העולים של האונייה "ירושלים", 15.1.1956', '#src-6'),
        ('ילקוט הפרסומים 1920 (1973), עמ\' 1710 — נפת צפת, ענף אבוטבול', '#src-12'),
