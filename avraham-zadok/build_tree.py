...
c_ag = 1100
s,_ = couple(c_ag, yG, "יוסף ארואס", "שלוש הצהרות גיל: ~1845 (1869) · ~1849 (1886) · ~1852 (1893)|צראף · טופס 1893: «בן Solomon Arwas»", "גרסיה ארואס", "אם מרים · ב-1893 רשומה שרה (29)", "arwas"); S.append(s)
x_avf = 1440
...
gaza_c = 428
S.append(box(gaza_c, yG, "בית אליהו בעזה — 1893", "תעודת נתין בריטי 24, קונסוליית יפו|«Father’s Name: Shalom Arrwas (dead)»|רעייתו פרו (36)|משה 17 · רחל 14 · מרים 12|סולטנה 10 · יעקב 4 · שלמה 2|— ובפנקס יפו 10.2.1886: אותו בית —|אליהו 30 · אורו 25 · משה 12 · רחל 10|מרים 7 · סולטנה 5 · (גיל ומגורים סותרים)", "arwas", w=286, h=176))
...
aharon_c = 596
S.append(box(aharon_c, y1, "אהרון צדוק", "~1895 - 18.11.1981 · יליד צנעא|עלה 1914 · יפו 1918 · מקווה מ-1916*|אופה ראשי 1920-1960 · סופר ופעיל ציבור", "parent"))
...
S.append(box(1635, 768, "הענף שנשאר במצרים — 1904-1950", "רפאל בן מיכאל · יליד יפו ~1904|(טופס 33, יפו 31.1.1906 — «Raphael, 2»)|היה כבן עשר בגירוש נתיני בריטניה 1914|בניו שמעון (1934) ויוסף (17.7.1935) — קהיר|«שם האב: רפאל» · עלו ארצה 1950|זיהוי האב — ככל הנראה", "arwas", w=284, h=120))
...
for i,ln in enumerate([
  ...
  'אמה של רוזה — מרים ארואס (מאומת: דף הרישום והמצבה; ראו מקרא) · שימו לב: בקורפוס שלוש נשים בשם מרים ארואס — שלנו (ילידת 1875), בתו של אליהו שבבית עזה (~1881), ובת בית יוסף בפנקס 1886 (~1868/9) — ושתי האחרונות אינן אמה של רוזה']):
...
<div class="legend">
 ...
 <ul>
  ...
  <li><b>אשכול 10.2.1886 (מהדורה 69).</b> פנקס נתיני בריטניה של המחוז הקונסולרי של יפו (ארכיון המדינה, סל-1544/1) רושם באותו יום <b>שלושה משקי בית ארואס ברצף — שבע-עשרה נפשות</b>: אליהו (30, חלפן) ובני ביתו, יוסף (37, ⇒ יליד ~1849 — הצהרת גיל שלישית) ובני ביתו, ומיכאל (20, חלפן) ובני ביתו. זהו האשכול הגדול ביותר בקורפוס. <b>סתירות שנרשמות ואינן מיושבות:</b> אליהו רשום בן 34 בטופס עזה מ-4.12.1885 ובן 30 עשרה שבועות אחר כך; מגוריו עזה שם ויפו כאן; ורעייתו «פרו» ב-1893 מול «אורו» ב-1886. וכן: «מרים ארואס» בת 17 שבבית יוסף היא ילידת ~1868/9 — <b>אינה</b> מרים ילידת 1875 שבעץ.</li>
  <li><b>הענף המצרי (מהדורה 55).</b> שני שאלוני רישום לעולה שנחתמו בחיפה ב-1950 מעמידים את <b>יוסף</b> (יליד קהיר 17.7.1935) ואת אחיו <b>שמעון</b> (יליד 1934; שדה התאריך נכתב «31/6» — תאריך שאינו קיים), <b>שניהם בני רפאל</b>, אזרחים מצרים. רפאל בן מיכאל, יליד יפו ~1904, היה כבן עשר בגירוש נתיני בריטניה מיפו ב-1914 — ומכאן, ככל הנראה, הענף שנשאר במצרים ושבניו שבו ארצה שלושים ושש שנים אחר כך. השרשרת שלמה ← מיכאל ← רפאל <b>מאומתת</b> בשלושה טפסים; זיהוי «רפאל» שבשאלונים — <b>ככל הנראה</b>.</li>
 ...
 <ul>
  ...
  <li><b>צד האב</b> — זיכרונות אהרון צדוק ומכתבו לנכדו (מקור ראשוני): לידה בצנעא (~1895-1898, המקורות חלוקים; בתיבה ננקב ~1895 על פי גיל 86 שברשומת הקבורה — מהדורה 38 — ובספר), משפחת צאלח, פטירת האב בעדן, רעב 1904, עלייה 1914. שם הסב אברהם — פנקס הבוחרים 1949 וכתובת אהרן ורוזה (מסמך ראשוני, מהדורה 35) — מאומת; שם הסבתא סעדה — MyHeritage + ספר המשפחה (שני מקורות משפחתיים לא בלתי-תלויים), ככל הנראה; האח יוסף, שנספה עם אמו — ספר המשפחה. <b>מהדורה 59:</b> רשומת פנקס תושבי יפו 1918-1919 (משק בית 685/2) נקראה אות-אות מן הסריקה — «צאלח · אהרן · רוק · 23», ובנוסף העדה «תימני» והמקצוע «פועל»; זו הרשומה החיצונית המוקדמת ביותר של אהרון, והזיהוי עלה ל<b>כמעט ודאי</b>. <b>מהדורה 60:</b> <i>(*שתי השנים בתיבה מתנגשות ונרשמות כך בכוונה: פנקס תושבי יפו 1918-1919 רושם אותו תושב יפו, ואילו הראיון ב«למרחב» גוזר הגעה למקווה ב-1916; מקווה ישראל סמוכה ליפו ותחום הרישום של ועד העיר לא נבדק — ראו ערכי מקור 28 ו-68.)</i> סריקת JPress העלתה חמישה-עשר פריטי עיתונות מ-1929 עד 1960; צוואת יהושע מרגולין («דבר», 10.10.1947) נוקבת «אהרון צדוק התימני, <b>האופה אשר במקוה-ישראל</b>» — עוגן הזיהוי — וראיון ב«למרחב» (1960) נותן 44 שנים במקווה, אופה ראשי מ-1920. חמישה טקסטים פרי עטו, ובראשם «בחדר בתימן» («דבר», 15.12.1933), עדות בגוף ראשון על החדר בצנעא.</li>
